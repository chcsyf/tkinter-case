...
![image-20230203234953996](./assets/image-20230203234953996.png)
...
界面对象为单独框架，而不是根程序`tk.Tk()`。

**2 计算器主要框架**

程序中主要创建五类组件对象，其中主界面、显示器、按键区三个均为`tk.Frame`组件，使用`tk.Canvas`亦可。

![image-20230203234845342](./assets/image-20230203234845342.png)



**3 使用集合创建重复对象。**

在创建多个`tk.Button`、`tk.Menu`、`tk.Canvas`的几何图元 等对象时，可以使用`list`或`dict`存储组件参数及创建的组件对象，以方便管理。

**4 计算器主要逻辑**

本程序使用最简单逻辑，即把输入作为字符串，使用`eval()`函数进行计算。注意，在按键时根据按键的类型和对象，适当判别按键的有效性，以提高算式的容错率。
