...
class MyServer:
    def __init__(self):
        ...
        # 保存玩家房间信息 name:int
        self.room_id = {}
        # 保存对局数据 int:[None,None,gobang]
        self.game_data = {}
        self.game_moving = {} # 当前状态
        self.game_player = {} # 当前出棋方
        self.thread = []
        threading.Thread(target=self.win).start()

    def win(self):
        win = tk.Tk()
        win.geometry('800x400')
        text_win = tk.Text(win)
        text_win.pack(expand='yes', fill='both')
        scroll = tk.Scrollbar(text_win) # 滚动条
        text_win.config(yscrollcommand=scroll.set)
        scroll.config(command=text_win.yview)
        ...
        def change_run(event):
            for th in self.thread:
                try:
                    ...
                    pass
            self.server.close()
        text_win.bind('<Destroy>', change_run)
        self.text = text_win
        tk.mainloop()

    def text_add(self, txt):
        if self.text:
    ...
    def run(self):
        while True:
            ...
            # 创建线程，负责接收客户端信息
            thread = multi_thread(target=self.recv_info, args=(socket,))
            thread.start()
            self.thread.append(thread)

    def send_msg(self, socket, data):
        '''发送消息'''
        self.text_add(f'发送消息：{data}')
    ...
    def recv_info(self, socket):
        while True:
            ...
            self.text_add(f'{self.socket_id[socket]}:{data}')
            if data == 'goquit':
                ...
                if data == 'room_new1' or data == 'room_new2':
                    ...
                    self.text_add(f'{self.socket_id[socket]}创建房间：{room}')
                elif data[:7] == 'room_in':
                    try:
                        r = int(data[7:])
                        ...
                        if self.game_data[r][0][0] is None:
                            ...
                            break
                        self.text_add(f'{self.socket_id[socket]}加入房间：{r}')
                    except:
                        self.socket_id.pop(socket)
                        self.send_msg(socket, 'out')
                        break
