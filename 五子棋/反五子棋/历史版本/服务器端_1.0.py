...
class MyServer:
    ...
    def run(self):
        while True:
            ...
            # 创建线程，负责接收客户端信息
            threading.Thread(target=self.recv_info, args=(socket,)).start()
    ...
    def recv_info(self, socket):
        while True:
            if self.socket_id[socket] is None:
                self.send_msg(socket, '请输入姓名')
            buff = socket.recv(4)
            if len(buff) != 4:
                continue
            ...
            if data[:5] == 'room_':
                ...
                # 接收到：自动匹配
                # for r in self.game_data:
                #     self.room_id[socket] = r
                #     if self.game_data[r][0][0] is None:
                #         self.send_msg(socket, f'{room}:{1}')
                #         self.game_data[r][0][0] = socket
                #         break
                #     elif self.game_data[r][0][1] is None:
                #         self.send_msg(socket, f'{room}:{2}')
                #         self.game_data[r][0][1] = socket
                #         break
                continue
            elif data[:4] == 'put_':
