...
class OppositeGobangUI:
    ...
    def __init__(self, parent):
        ...
        # 注释框
        notes_win = tk.Text(win_info, height = 7)
        win_info.add(notes_win)

        # 历史框
        self.record_win = tk.Canvas(win_info, height = self.cell*7, bg='whitesmoke')
    ...
    def draw_back(self):
        ...
        if not self.record[0]:
            return
        if self.record[self.step[0]]:
            st = self.step[0]
        else:
            st = self.step[0][:-1]
            while not self.record[st]:
                st = st[:-1]
        self.restep(st, '-')
    ...
    def draw_go(self, data):
        ...
        for step,pts in data.items():
            step = 0 if step == '0' else tuple(step)
    ...
    def opt_pts(self, pts, color):
        ...
        self.canvas.delete('aims')
        for x,y in pts:
            r = 6; p1 = self.repair((x,y), -r); p2 = self.repair((x,y), r)
    ...
    def wheel_record(self, event):
        ...
        if not self.piece[0]:
            return
        if self.piece[self.step[0]]:
            st = self.step[0]
        else:
            st = self.step[0][:-1]
            while not self.piece[st]:
                st = st[:-1]
        if event.delta > 0:
            self.restep(st, '-')
            for i in self.piece[st].pop():
                self.canvas.delete(i)
            info = self.record[st][len(self.piece[st])-1]
            r = 6; r2 = 5 # 标识半径
            if len(info) == 3:
                pt = info[:2]
                pt1 = self.repair(pt, -r); pt2 = self.repair(pt, r)
                self.canvas.create_oval(*pt1, *pt2, fill='red',
                        outline='', tags='last')
            elif len(info) == 6:
                pt = info[:2]
                pt1 = self.repair(pt, -r); pt2 = self.repair(pt, r)
                self.canvas.create_oval(*pt1, *pt2, fill='green', outline='', tags='last')
                pt = info[3:5]
                pt1 = self.repair(pt, -r); pt2 = self.repair(pt, r)
                self.canvas.create_oval(*pt1, *pt2, fill='red', outline='', tags='last')
        elif event.delta > 0:
            self.restep(st, '+')
            info = self.record[st][len(self.piece[st])]
